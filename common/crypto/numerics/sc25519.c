...
static const UNMontgomeryConstants256 sc25519_scalarMontgomeryPrecalc = {
    {0x01, 0x0f, 0x9c, 0x44, 0xe3, 0x11, 0x06, 0xa4, 0x47, 0x93, 0x85, 0x68,
     0xa7, 0x1b, 0x0e, 0xd0, 0x65, 0xbe, 0xf5, 0x17, 0xd2, 0x73, 0xec, 0xce,
     0x3d, 0x9a, 0x30, 0x7c, 0x1b, 0x41, 0x99, 0x03, 0xed, 0xd3, 0xf5, 0x5c,
     0x1a, 0x63, 0x12, 0x58, 0xd6, 0x9c, 0xf7, 0xa2, 0xde, 0xf9, 0xde, 0x14,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x10, 0x1b, 0x7e, 0x54, 0x12}};

#define uint32_scalar \
  sc25519_scalarMontgomeryPrecalc.constants.prime.as_uint32_t

#endif

static int32_t bigint_sub(uint32_t* dest, const uint32_t* sub,
...
static int32_t bigint_add(uint32_t* dest, const uint32_t* addend,
                          uint32_t numberOfWords) {
  unsigned ctr;
  int64_t accu = 0;
...
static void sc25519_reduceAddSub(sc25519* valueToReduce,
                                 sc25519* tempBufferSuppliedByCaller) {
  int32_t didUnderflowHappen;

  cpy_256bitvalue(tempBufferSuppliedByCaller, valueToReduce);
  didUnderflowHappen =
      bigint_sub(tempBufferSuppliedByCaller->as_uint32_t, uint32_scalar, 8);
...
static void sc25519_reduceAddSub(sc25519* valueToReduce,
  ...
  didUnderflowHappen =
      montgomery_mnac_U16(valueToReduce->as_uint32_t, 1, uint32_scalar);
  montgomery_mac_lowU16(valueToReduce->as_uint32_t,
                        didUnderflowHappen * didUnderflowHappen, uint32_scalar);
}

#endif

void sc25519_from32bytes(sc25519* r, const uint8_t x[32]) {
  UN_512bitValue temp;
  uint8_t ctr;
...
void sc25519_from64bytes(sc25519* r, const uint8_t x[64]) {
  UN_512bitValue temp;
  uint8_t ctr;
...
void sc25519_to32bytes(uint8_t r[32], const sc25519* x) {
  uint8_t ctr;
...
void sc25519_add(sc25519* result, const sc25519* addend1,
...
void sc25519_sub(sc25519* result, const sc25519* addend,
  ...
  didUnderflowHappen =
      bigint_sub(result->as_uint32_t, valueToSubstract->as_uint32_t, 8);
  {
    int i;
    for (i = 0; i < 8; i++) {
...
void sc25519_reduce(UN_512bitValue* valueToReduce) {
  /* use Barret reduction. Multiply with 288 bits so that all intermediate
     results are also aligned to four-byte boundaries. */
  static const uint32_t u32_mu[9] = {0x0a2c131bUL, 0xed9ce5a3UL, 0x086329a7UL,
                                     0x2106215dUL, 0xffffffebUL, 0xffffffffUL,
                                     0xffffffffUL, 0xffffffffUL, 0xfUL};
  ...
  multiply288x288(&intermResultBarret, u288_mu, valueForBarretMpy);
  {
    ...
    multiply288x288(&valueToSubstract, approximateResultOfDivision, u288_pr252);

    bigint_sub(valueToReduce->as_uint32_t, valueToSubstract.as_uint32_t, 16);
    /* due to rounding the true result of the division may be wrong by one (or
       two?). we need to conditionally substract the prime if the result of the
       substraction is nonnegative. */
  }
...
void sc25519_reduce(UN_512bitValue* valueToReduce) {
  ...
  montgomery_reduce(&tmp, valueToReduce,
                    &sc25519_scalarMontgomeryPrecalc.constants);
  multiply256x256(valueToReduce, &tmp,
                  &sc25519_scalarMontgomeryPrecalc.constants.kSquare);
  montgomery_reduce(&valueToReduce->as_256_bitValue[0], valueToReduce,
                    &sc25519_scalarMontgomeryPrecalc.constants);
}
...
void sc25519_sqr(sc25519* r, const sc25519* x) {
  UN_512bitValue temp;
...
static const UN_256bitValue numberOfPointsOnCurve25519 = {
    {0x68, 0x9f, 0xae, 0xe7, 0xd2, 0x18, 0x93, 0xc0, 0xb2, 0xe6, 0xbc,
     0x17, 0xf5, 0xce, 0xf7, 0xa6, 0x0,  0x0,  0x0,  0x0,  0x0,  0x0,
     0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x80}};

void sc25519_calculateRandomMultipleOfGroupOrder(UN_288bitValue* result) {
  uint32_t multiplier;
...
void sc25519_window4(signed char r[SC25519_WINDOW4_SIZE], const sc25519* s) {
  char carry;
  uint8_t i;
  ...
  randomAddend.as_uint32_t[8] +=
      bigint_add(randomAddend.as_uint32_t, s->as_uint32_t, 8);
  ...
  /* Making the result signed and limited to the range -8 ... +8 for the signed
   * NAF form. */
  carry = 0;
...
// binary extended gcd algorithm based on Alg. 14.61 in the Handbook of Applied
// Cryptography
void sc25519_binary_extended_gcd(UN_256bitValue* R, const UN_256bitValue* X,
...
void sc25519_inverse(UN_256bitValue* R, const UN_256bitValue* X) {
  UN_256bitValue order = {{0xed, 0xd3, 0xf5, 0x5c, 0x1a, 0x63, 0x12, 0x58,

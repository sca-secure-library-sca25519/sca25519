...
FORCE_INLINE static uint64_t multiply32x32(uint32_t x, uint32_t y);

// static uint32_t FORCE_INLINE
// multiply32x32_upperWord (uint32_t x, uint32_t y);

FORCE_INLINE void multiply32x64(UN_96bitValue* result, uint32_t x,
                                const uint64_t* py);

FORCE_INLINE void multiply64x64(UN_128bitValue* r, const uint64_t* x,
                                const uint64_t* y);

FORCE_INLINE static void square64(UN_128bitValue* r, const uint64_t* pu64_x);
...
FORCE_INLINE static void multiply64x64_paramStruct(ST_mpy64x64_params* params);

static uint64_t multiply32x32(uint32_t x, uint32_t y) {
  uint64_t r = ((uint32_t)((uint16_t)x)) * ((uint16_t)y) |
               (((uint64_t)((x >> 16) * (y >> 16))) << 32);
...
void multiply32x64(UN_96bitValue* result, uint32_t x, const uint64_t* py) {
...
/// Multiply two 64 bit values with a 128 bit result. Uses the karatsuba method.
/// Is faster than the texbook variant mainly because less memory accesses are
/// needed (2 cycles on Cortex M0).
void multiply64x64(UN_128bitValue* r, const uint64_t* pu64_x,
...
/// Is faster than the texbook variant mainly because less memory accesses are
/// needed (2 cycles on Cortex M0).
FORCE_INLINE void multiply64x64_paramStruct(ST_mpy64x64_params* params) {
  // This call will be inlined. The compiler will exploit the fact that all data
  // may be accessed by one single pointer.
  multiply64x64(&params->result, &params->x.as_uint64_t[0],
                &params->y.as_uint64_t[0]);
}

FORCE_INLINE void square64(UN_128bitValue* r, const uint64_t* pu64_x) {
  uint32_t* au32_x = (uint32_t*)pu64_x;
  uint16_t* au16_x = (uint16_t*)au32_x;
  ...
  r->as_uint32_t[3] =
      ((uint32_t)(accu >> 32)) + ((uint32_t)au16_x[3]) * au16_x[3];
}

#ifndef CRYPTO_HAS_ASM_MPY_96

void multiply96x96_c(UN_192bitValue* result, const UN_96bitValue* x,
  ...
  multiply64x64(&result->as_128_bitValue[0], &x->as_uint64_t[0],
                &y->as_uint64_t[0]);
...
void square96_c(UN_192bitValue* result, const UN_96bitValue* x) {
  UN_96bitValue tmp;
  uint8_t ctr;
  uint64_t accu = 0;
...
void square128_c(UN_256bitValue* result, const UN_128bitValue* x) {
  square64(&result->as_128_bitValue[0], &x->as_uint64_t[0]);
...
void multiply128x128_c(UN_256bitValue* result, const UN_128bitValue* x,
  ...
  // Now accumulate the results.
  {
    ...
    result->as_uint32_t[6] = (uint32_t)(accu);
    result->as_uint32_t[7] =
        ((uint32_t)(accu >> 32)) + m2.result.as_uint32_t[3];
  }
}
...
void multiply192x192_c(UN_384bitValue* result, const UN_192bitValue* x,
...

  {
    ...
    {
      // use inverted sign in comparison to deltax, so that we may always add
      // the value
...

    {
      ...
      multiply96x96(&temp, &deltaX, &deltaY);
      {
        ...
        for (ctr = 0; ctr < 3; ctr++) {
          ...
          accu +=
              temp.as_uint32_t[ctr];  // lower half term of difference product
          temp.as_uint32_t[ctr] =
              (uint32_t)accu;  // store the result temporary here since we will
                               // need the content of the result buffer.
          accu >>= 32;
        }

        for (ctr = 0; ctr < 3; ctr++) {
          accu += temp.as_uint32_t[ctr +
                                   3];  // upper half term of difference product
...
void square192_c(UN_384bitValue* result, const UN_192bitValue* x) {
  // dispatcher == either the ASM or C variant is chosen.
...

  {
    ...
    // dispatcher == either the ASM or C variant is chosen.
    multiply96x96(&temp, (const UN_96bitValue*)&x->as_uint32_t[0],
                  (const UN_96bitValue*)&x->as_uint32_t[3]);
...
void multiply256x256_c(UN_512bitValue* result, const UN_256bitValue* x,
  ...
  // dispatcher == either the ASM or C variant is chosen.
  multiply128x128(&result->as_256_bitValue[0], &x->as_128_bitValue[0],
                  &y->as_128_bitValue[0]);
  multiply128x128(&result->as_256_bitValue[1], &x->as_128_bitValue[1],
                  &y->as_128_bitValue[1]);

  {
...

    {
      ...
      multiply128x128(&temp, &deltaX, &deltaY);
      {
        ...
        for (ctr = 0; ctr < 4; ctr++) {
          accu += temp.as_uint32_t[ctr +
                                   4];  // upper half term of difference product
          accu -= deltaY.as_uint32_t[ctr] & ((uint32_t)upperWordDeltaX);
          accu -= deltaX.as_uint32_t[ctr] & ((uint32_t)upperWordDeltaY);
...
void square256_c(UN_512bitValue* result, const UN_256bitValue* x) {
  // dispatcher == either the ASM or C variant is chosen.
...

  {
    ...
    // dispatcher == either the ASM or C variant is chosen.
    multiply128x128(&temp, &x->as_128_bitValue[0], &x->as_128_bitValue[1]);
...
/// Will be required for the barret reduction for the "scalar" prime modulo
/// calculations.
void multiply288x288(UN_576bitValue* r, const UN_288bitValue* x,
                     const UN_288bitValue* y) {
  multiply256x256(&r->as_512_bitValue[0], &x->as_256_bitValue[0],
                  &y->as_256_bitValue[0]);
...
void multiply136x136(UN_272bitValue* r, const UN_136bitValue* x,
                     const UN_136bitValue* y) {
  uint64_t accu;

  multiply128x128(&r->as_256_bitValue[0], &x->as_128_bitValue[0],
                  &y->as_128_bitValue[0]);

  {
    ...
    for (ctr = 0; ctr < 4; ctr++) {
      ...
      accu += ((uint64_t)(xmax * ((uint32_t)y->as_uint16_t[1 + 2 * ctr])))
              << 16;
      accu += ymax * ((uint32_t)x->as_uint16_t[2 * ctr]);
      accu += ((uint64_t)(ymax * ((uint32_t)x->as_uint16_t[1 + 2 * ctr])))
              << 16;
...
void setone_256bitvalue(UN_256bitValue* dest) {
  uint32_t ctr;
...
void setzero_256bitvalue(UN_256bitValue* dest) {
  uint32_t ctr;
...
void cpy_256bitvalue(UN_256bitValue* dest, const UN_256bitValue* source) {
  uint32_t ctr;
...
void cpy_192bitvalue(UN_192bitValue* dest, const UN_192bitValue* source) {
  uint32_t ctr;
...
void conditionalMove_192bitValue(UN_192bitValue* r, const UN_192bitValue* x,
                                 uint8_t b) {
  int32_t mask = b;
  uint32_t ctr;
...
/// Gets an uint8_t as third parameter that shall be zero or one.
void conditionalMove_256bitValue(UN_256bitValue* r, const UN_256bitValue* x,
                                 uint8_t b) {
  int32_t mask = b;
  uint32_t ctr;

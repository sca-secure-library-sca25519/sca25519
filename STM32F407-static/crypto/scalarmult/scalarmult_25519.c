...
extern void curve25519_cswap_asm(ST_curve25519ladderstepWorkingState* state, uint32_t* b);

#define ROTATE16(a) { a = (((uint32_t) a) >> 16) | (a << 16); }

#define HAS_ASM_cSwapAndRandomize_asm

#ifdef HAS_ASM_cSwapAndRandomize_asm
extern void cSwapAndRandomize_asm (
                        uint32_t swapData,
                        uint32_t *pFe1,
                        uint32_t *pFe2,
                        uint32_t randomVal);
                        ...
                        uint32_t randomVal)
{
   ...
   ROTATE16 (randomize_mpy); scaledA = ((uint64_t) randomize_mpy) * outA;
   ROTATE16 (randomize_mpy); scaledB = ((uint64_t) randomize_mpy) * outB;
...
#define ROTATER(a,cnt) { a = (((uint32_t) a) >> cnt) | (a << (32 - cnt)); }

static void maskScalarBitsWithRandomAndCswap(
      ST_curve25519ladderstepWorkingState *pState,
      uint32_t wordWithConditionBit,
      uint32_t bitNumber) 
{ 
    uint32_t randomDataBuffer[2] = {0,0};
    randombytes (randomDataBuffer, sizeof(randomDataBuffer));
    //
    ...
    cSwapAndRandomize (wordWithConditionBit, 
                       pState->xp.as_uint32_t, pState->xq.as_uint32_t, 
                       randomDataBuffer[1]);
    cSwapAndRandomize (wordWithConditionBit, 
                       pState->zp.as_uint32_t, pState->zq.as_uint32_t, 
                       randomDataBuffer[1]);
}
...
static const fe25519 CON486662 = {{0x06, 0x6d, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00,
                                   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
                                   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
                                   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};


static void curve25519_doublePoint(
    point25519* R,
    const point25519* P
)
{
    // x3 = 2y1z1^2 * (3x1^2z1+2ax1z1^2+z1^3)^2 - 8ay1^3z1^6 - 16x1y1^3z1^5
    // y3 = 4y1^2z1^3 * (3x1+az1) * (3x1^2z1+2ax1z1^2=z1^3) - (3x1^2z1+2ax1z1^2=z1^3)^3 - 8y1^4z1^5
...
{
    // x3 = (y2z1 - y1z2)^2 * z1z2*(x2z1 - x1z2) - (x2z1-x1z2)^3*(a*z1z2 + x1z2 + x2z1)
    // y3 = ((2*x1z2 + x2z1) + a*z1z2) * (y2z1 - y1z2) * (x1z2 - x1z2)^2 - z1z2*(y2z1 - y1z2)^3 - y1z2*(x2z1 - x1z2)^3
    // z3 = z1z2*(x2z1 - x1z2)^3
...
static void
computeY_curve25519_projective
(
    fe25519* y1,
    fe25519* x1, fe25519* z1, // (x1,z1) = k*P
...
static int
computeY_curve25519_affine(fe25519* y ,const fe25519* x) {
    // y^2 = x^3 + 486662x^2 + x
    fe25519 tmp, x2;
...
void set_static_key_curve25519(
    const uint8_t* uRx,
    const uint8_t* uRy,
    const uint8_t* uRz,
    const uint8_t* uSx,
...
void update_static_key_curve25519() {
    ...
    R0.x = &rx0; R0.y = &ry0; R0.z = &rz0;
    S0.x = &sx0; S0.y = &sy0; S0.z = &sz0;
...
#define INCREMENT_BY_NINE(a) { a += 9; }

#define INCREMENT_BY_163(a) { a += 163; }
...
)
{
    ...
    point25519 P, R;
    P.x = &state.xp; P.y = &yp; P.z = &state.zp;
    R.x = &Rx; R.y = &Ry; R.z = &Rz;

    if(computeY_curve25519_affine(&yp, &state.x0) != 0) {
    	goto fail;
    }
    curve25519_addPoint(&P, &P, &R);

    // Double 3 times before we start
    ...
    sc25519_mul(&t, &state.r, &randB);
    sc25519_inverse(&t, &t);

    sc25519_mul(&Rinv, &t, &randB);
    sc25519_mul(&state.s, &state.s, &Rinv);
#else
    ...
    while (state.nextScalarBitToProcess >= 0)
    {
...
#ifdef MULTIPLICATIVE_CSWAP
        {
          ...
          maskScalarBitsWithRandomAndCswap (&state,state.s.as_uint32_t[limbNo],bitNo);
        }
#else
        {
          limbNo = (uint8_t)(state.nextScalarBitToProcess >> 5);
...
#ifdef MULTIPLICATIVE_CSWAP
#ifdef ITOH_COUNTERMEASURE
          maskScalarBitsWithRandomAndCswap (&state, itohShift.as_uint32_t[limbNo], bitNo);
#endif
#else 
#ifdef ITOH_COUNTERMEASURE
    ...
    while (state.nextScalarBitToProcess >= 0)
    {
...
#ifdef MULTIPLICATIVE_CSWAP
        {
          ...
          maskScalarBitsWithRandomAndCswap (&state,state.r.as_uint32_t[limbNo],bitNo);
        }
#else
        {
          limbNo = (uint8_t)(state.nextScalarBitToProcess >> 5);
...
#ifdef MULTIPLICATIVE_CSWAP
#ifdef ITOH_COUNTERMEASURE64
           maskScalarBitsWithRandomAndCswap (&state, itoh64Shift.as_uint32_t[limbNo], bitNo);
#endif
#else 
#ifdef ITOH_COUNTERMEASURE64
    ...
    point25519 A;
    fe25519 Ay;
    A.x = &Sx; A.y = &Ay; A.z = &Sz;
    P.x = &state.xp; P.y = &yp; P.z = &state.zp;
    fe25519_cpy(A.y, &Sy);
    fe25519_neg(A.y, A.y);
    curve25519_addPoint(&P, &P, &A);
